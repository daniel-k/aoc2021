...
import argparse
from utils import pairwise, triplets
...
inputs = [int(line) for line in args.INPUT]

if not args.part_two:
    depths = inputs
else:
    depths = [sum(x) for x in triplets(inputs)]

deeper_than_last = [b > a for a, b in pairwise(depths)]

print(sum(deeper_than_last))