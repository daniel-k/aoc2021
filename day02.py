...
steps = [
    tuple(d * int(magnitude) for d in directions[dir_])
    for dir_, magnitude in [line.split() for line in args.INPUT]
]
if not args.part_two:
    def step_reducer(state, step):
        # directions: (horiz, depth)
        return (state[0] + step[0], state[1] + step[1])

    initial = (0, 0)
else:
    def step_reducer(state, step):
        horiz, depth, aim = state

        # directions: (horiz, aim)
        go_foward, change_aim = step
...
destination = reduce(step_reducer, steps, initial)
